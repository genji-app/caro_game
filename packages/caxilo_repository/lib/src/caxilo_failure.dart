...
sealed class CaxiloFailure implements Exception {
  ...
  const CaxiloFailure({this.source});
  ...
  @override
  String toString() => '$runtimeType(source: $source)';
}
...
final class CaxiloServerFailure extends CaxiloFailure {
  const CaxiloServerFailure({super.source});
...
final class CaxiloBusinessFailure extends CaxiloFailure {
  const CaxiloBusinessFailure({required this.message, super.source});

  /// The operator-defined error message from the API response.
  final String message;
}

/// Catch-all for errors that do not match any known domain type.
final class CaxiloUnknownFailure extends CaxiloFailure {
  const CaxiloUnknownFailure({super.source});
}
...
CaxiloFailure mapToCaxiloFailure(Object error) {
  ...
  if (error is gac.GameApiException) {
    return switch (error.type) {
      ...
      gac.GameApiExceptionType.serverError => CaxiloServerFailure(source: error),
      ...
      _ => CaxiloUnknownFailure(source: error),
    };
  }
