...
class AdaptiveOverlayNavigator extends InheritedWidget {
  ...
  Future<NavigatorState?> _prepareNavigator() async {
    ...
    // Fast path: Navigator already mounted
    final existingState = navigatorKey.currentState;
    if (existingState != null) return existingState;

    // Wait for Navigator to mount after overlay opens.
    // Uses post-frame callbacks instead of fixed delay for precision.
    ...
    void checkMount(Duration _) {
      ...
      if (state != null) {
        ...
        // Navigator chưa mount sau 1 frame → thử thêm 1 frame nữa
