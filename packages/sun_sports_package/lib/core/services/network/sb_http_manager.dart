...
import 'package:sun_sports/core/services/models/play_history_model.dart';
import 'package:sun_sports/core/services/models/sun_api_response.dart';
import 'package:sun_sports/core/services/models/transaction.dart';
...
class SbHttpManager {
  ...
  Future<List<Map<String, dynamic>>> getNotifications() async {
    ...
    // API may return an empty body ("") when there are no notifications.
    // In that case `send` returns the raw String, so we treat it as empty
    // instead of failing the cast to Map<String, dynamic>.
  ...
  Future<LivestreamResponse> getLiveLink(String eventId, String brand) async {
    ...
    final response =
        await send(url, json: true, headerToken: true) as Map<String, dynamic>;
    // API trả h5Link dạng path tương đối -> bọc bằng domain player
    // ({videoDomain}/?link={encoded}&size=75) để iframe/WebView phát được.
    return LivestreamResponse.fromJson(response).wrapWithPlayerDomain(
      videoDomain: urlVideoJS,
      virtualVideoDomain: urlVirtualVideoJS,
    );
  }
