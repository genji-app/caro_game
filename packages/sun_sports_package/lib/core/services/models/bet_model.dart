...
/// Parse mảng `values` của response 603 = [giá client gửi, giá server đang có].
/// Xử lý cả phần tử dạng num (`[1.97, 1.96]`) lẫn String (`["1.91","1.9"]`).
...
extension PlaceBetResponseX on PlaceBetResponse {
  ...
  /// Giá odds hiện tại của server khi bị lỗi 603 "Odds have changed".
  /// `values` = [giá client gửi, giá server đang có] → trả phần tử cuối.
  /// Dùng để retry place-bet với đúng giá server.
