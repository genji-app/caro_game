...
import 'package:sun_sports/core/services/providers/league_provider.dart';
...
import 'package:sun_sports/features/profile/deposit/domain/notifiers/deposit_notifiers.dart';
import 'package:sun_sports/features/profile/deposit/domain/notifiers/bank_submit_notifier.dart';
import 'package:sun_sports/features/profile/deposit/domain/notifiers/bank_transaction_slip_notifier.dart';
import 'package:sun_sports/features/profile/deposit/domain/notifiers/card_submit_notifier.dart';
import 'package:sun_sports/features/profile/deposit/domain/notifiers/codepay_qr_timer_notifier.dart';
import 'package:sun_sports/features/profile/deposit/domain/notifiers/codepay_submit_notifier.dart';
import 'package:sun_sports/features/profile/deposit/domain/notifiers/crypto_submit_notifier.dart';
import 'package:sun_sports/features/profile/deposit/domain/notifiers/giftcode_submit_notifier.dart';
import 'package:sun_sports/features/profile/deposit/domain/notifiers/verify_bank_submit_notifier.dart';
...
/// Whether the deposit config is being loaded for the first time
/// (no cached data available yet).
///
/// Used by payment-methods grid widgets to decide whether to render a
/// shimmer placeholder instead of the actual grid. Showing the real grid
/// before the config arrives caused a 6 → 4 flicker because optional tabs
/// (Bank, e-Wallet) initially defaulted to visible and then disappeared
/// once the API returned empty lists.
///
/// Behavior:
///   - first load (no cached value, request in flight) → true (show shimmer)
///   - background refresh of cached value → false (keep showing real grid)
///   - data available → false
///   - error (no cached value) → false (fall back to grid; optional tabs
///     show by default on error per [hasBankAccountsProvider] semantics)
final isDepositConfigLoadingProvider = Provider.autoDispose<bool>((ref) {
  final asyncConfig = ref.watch(configDepositProvider);
  return asyncConfig.isLoading && !asyncConfig.hasValue;
});
...
/// Whether the deposit config exposes at least one crypto option.
/// Used by both the deposit and withdraw UIs to decide whether to render
/// the "Tiền điện tử" tab.
///
/// Behavior mirrors [hasBankAccountsProvider]:
///   - data + empty list → false (hide tab)
///   - data + non-empty list → true (show tab)
