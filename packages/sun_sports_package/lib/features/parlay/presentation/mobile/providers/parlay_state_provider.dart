...
import 'package:sun_sports/core/services/adapters/sport_socket_adapter_provider.dart';
...
import 'package:sun_sports/core/services/providers/auth_provider.dart';
import 'package:sun_sports/core/services/providers/user_provider/user_provider.dart';
import 'package:sun_sports/core/services/repositories/betting_repository.dart';
import 'package:sun_sports/core/services/storage/parlay_storage.dart';
...
class ParlayStateNotifier extends StateNotifier<ParlayState> {
  ...
  Future<AddBetResult> addSingleBetFromPopupData(
    ...
    // Đồng bộ odds với socket store trước khi validate/lưu phiếu — "thấy sao
    // đặt vậy", tránh 603 do popupData (snapshot) bị cũ.
    ...
    try {
      final request = CalculateBetRequest(
        ...
        oddsStyle: _getOddsStyleCode(syncedBet.oddsStyle),
      );
  ...
  /// Việc đồng bộ odds realtime được xử lý trong [addSingleBetFromPopupData].
  ...
  /// Đồng bộ odds của [bet] với socket data store — nguồn realtime DUY NHẤT
  /// (cùng nguồn server dựa vào để validate). Gọi ngay trước khi add-to-slip và
  /// trước khi place để tránh lỗi 603 "Odds have changed" do odds phiếu bị cũ.
  ///
  /// Ghi thẳng vào `oddsData` (giữ `updatedOdds = null`) → đặt im lặng, không
  /// bật banner "odds đã đổi". Nếu kèo bị suspended → trả bet với `isDisabled`.
  SingleBetData _syncBetToLiveOdds(SingleBetData bet) {
    final offerId = bet.offerId;
    if (offerId == null || offerId.isEmpty) return bet;
    // Bet disabled (607/suspended) không bao giờ được place vì `canPlaceBet`
    // yêu cầu `!isDisabled` → bỏ qua sync.
    ...
    // Kèo bị tạm khóa giữa chừng → đánh dấu disabled, place sẽ tự bỏ qua.
    if (odds.isSuspended) {
      return bet.copyWith(isDisabled: true);
    }

    final newValue = _getOddsValueFromLibrary(odds, bet.oddsType, bet.oddsStyle);
    ...
    if (newValue == null || newValue <= 0) return bet;

    // No-op nếu odds không đổi (dung sai floating-point).
    if ((newValue - bet.displayOdds).abs() < 0.001) return bet;

    return bet.copyWith(oddsData: _applyStoreOddsToBet(bet, odds));
  }

  /// Ghi đè odds trong `bet.oddsData` bằng giá realtime từ socket store, cho
  /// đúng [bet.oddsType]. `decimal` luôn lấy từ store (field quyết định payload);
  /// `malay/indo/hk` chỉ ghi đè khi store có giá parse được, ngược lại giữ
  /// nguyên giá gốc (tránh corrupt format — vd draw không có malay/indo/hk).
  ...
  /// Build [BetSelectionModel] cho place-bet API từ [bet].
  ///
  /// [overrideOdds]: nếu có → ghi đè displayOdds (dùng khi retry 603 với đúng
  /// giá server trả về). Single bet từ sport list luôn `oddsStyle = decimal`
  /// nên `toStringAsFixed(2)` đúng định dạng; nếu sau này hỗ trợ Malay/Indo/HK
  /// cần verify định dạng `values` của response 603 với backend.
  BetSelectionModel _buildPlaceSelection(
    ...
    return BetSelectionModel(
      ...
      offerId: bet.offerId ?? '',
      displayOdds: overrideOdds?.toStringAsFixed(2) ?? bet.displayOddsString,
      oddsStyle: _getOddsStyleCode(bet.oddsStyle),
      cls: bet.cls,
      ...
      stake: bet.stake.toDouble(), // VND (converted to int in toRequestJson)
      winnings: bet.potentialWinnings, // VND (rounded in toRequestJson)
    );
  }
  ...
  /// Đặt 1 cược single, track theo [selectionId] (ổn định hơn index — list có
  /// thể mutate giữa các lần thử).
  ///
  /// [overrideOdds]: null = attempt đầu (sync odds từ socket store); có giá trị
  /// = retry với đúng giá server trả về trong `values[1]` của lỗi 603.
  /// [retriesLeft]: số lần retry còn lại khi gặp 603.
  Future<bool> _placeSingleBetInternal(
    String selectionId, {
    double? overrideOdds,
    int retriesLeft = 1,
  }) async {
    // Tìm lại bet theo selectionId — chạy cả lần đầu lẫn lần retry.
    final index = state.singleBets.indexWhere(
      (b) => b.selectionId == selectionId,
    );
    if (index < 0) {
      // Bet đã bị xóa khỏi phiếu giữa chừng → thoát an toàn.
    ...
    // Attempt đầu (overrideOdds == null): sync odds từ socket store.
    if (overrideOdds == null) {
      singleBet = _syncBetToLiveOdds(singleBet);

      // Kèo bị tạm khóa giữa chừng → ghi lại state, không gọi API.
      if (singleBet.isDisabled) {
      ...
      // Ghi odds đã sync trở lại state để UI khớp giá thực tế khi đặt.
    ...
    try {
      ...
      // Auto-retry khi 603: gửi lại đúng giá server (`values[1]`) — server vừa
      // báo giá đang valid nên gửi lại ngay thường khớp. 603 = rejection dứt
      // khoát (bet chưa vào sổ) → resubmit an toàn, không double-place.
      if (response.errorCode == 603 && retriesLeft > 0) {
        final serverOdds = response.serverCurrentOdds;
        if (serverOdds != null) {
          await Future<void>.delayed(const Duration(milliseconds: 50));
          // StateNotifier có thể đã dispose nếu user thoát màn hình.
  ...
  Future<PlaceMultipleBetsResult> placeAllSingleBetsParallel() async {
    ...
    final futures = betsToPlace.map((rawBet) async {
      // Attempt đầu: đồng bộ odds với socket store (auto, không hỏi).
      final singleBet = _syncBetToLiveOdds(rawBet);

      // Kèo bị tạm khóa giữa chừng → không gọi API.
      if (singleBet.isDisabled) {
      ...
      // null = attempt đầu (odds sync từ store); có giá trị = retry với đúng
      // giá server trả về trong values[1] của lỗi 603.
      double? overrideOdds;

      // Tối đa 2 attempt: attempt 1 = store; attempt 2 = retry values[1].
      for (var attempt = 0; attempt < 2; attempt++) {
        ...
        // Chỉ retry khi 603 và server có trả values[1]; còn lại thoát vòng lặp.
